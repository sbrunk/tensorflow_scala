...
private[api] class INT32Tensor private[tensors] (
  ...
  extends Tensor[INT32.type](INT32, shape, buffer, order) {

  override private[api] def setElementAtFlattenedIndex(index: Int, value: Int): this.type = {
    dataType.putElementInBuffer(buffer, index * dataType.byteSize, value)
    this
  }

  override private[api] def getElementAtFlattenedIndex(index: Int): Int = {
