...
/**
  * @author Sören Brunk
  */
private[tensors] class UINT16Tensor (
  override val shape: Shape,
  override val buffer: ByteBuffer,
  override val order: Order = DEFAULT_TENSOR_MEMORY_STRUCTURE_ORDER)
  extends Tensor[UINT16.type](UINT16, shape, buffer, order) {

  override private[api] def setElementAtFlattenedIndex(index: Int, value: UShort): this.type = {
    dataType.putElementInBuffer(buffer, index * dataType.byteSize, value)
    this
  }

  override private[api] def getElementAtFlattenedIndex(index: Int): UShort = {
