...
/**
  * @author Sören Brunk
  */
private[tensors] class FLOAT32Tensor (
  override val shape: Shape,
  override val buffer: ByteBuffer,
  override val order: Order = DEFAULT_TENSOR_MEMORY_STRUCTURE_ORDER)
  extends Tensor[FLOAT32.type](FLOAT32, shape, buffer, order) {

  override private[api] def setElementAtFlattenedIndex(index: Int, value: Float): this.type = {
    dataType.putElementInBuffer(buffer, index * dataType.byteSize, value)
    this
  }

  override private[api] def getElementAtFlattenedIndex(index: Int): Float = {
