...
/**
  * @author Sören Brunk
  */
private[tensors] class BOOLEANTensor (
  override val shape: Shape,
  override val buffer: ByteBuffer,
  override val order: Order = DEFAULT_TENSOR_MEMORY_STRUCTURE_ORDER)
  extends Tensor[BOOLEAN.type](BOOLEAN, shape, buffer, order) {

  override private[api] def setElementAtFlattenedIndex(index: Int, value: Boolean): this.type = {
    dataType.putElementInBuffer(buffer, index * dataType.byteSize, value)
    this
  }

  override private[api] def getElementAtFlattenedIndex(index: Int): Boolean = {
